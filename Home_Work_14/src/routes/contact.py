...
async def get_contact(limit: int = Query(10, le=500), offset: int = 0, db: Session = Depends(get_db),
    ...
    :param le: Limit the number of contacts that can be returned at a time
...
def search_contacts(query: str, db: Session = Depends(get_db), current_user: User = Depends(auth_service.get_current_user)):
    try:
